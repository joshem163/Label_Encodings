    ...
    for run in range(args.runs):
        ...
        for epoch in range(1, 1 + args.epochs):
            ...
            if args.dataset in ['questions', 'minesweeper','tolokers']:
                result = test(model, data, train_idx, valid_idx, test_idx,args.model_type, metric='roc_auc')
            else:
                result = test(model, data, train_idx, valid_idx, test_idx,args.model_type, metric='accuracy')
...
if __name__ == "__main__":
    ...
    datasets_to_run=['cora','corafull','roman-empire']
    lr_values = [0.001]
